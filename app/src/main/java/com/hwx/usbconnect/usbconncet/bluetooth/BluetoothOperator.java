...
public class BluetoothOperator {
    ...
    public boolean connectForName() {
        ...
        BluetoothDevice device = searchPairedDevicesF(FILTER_NAME);
        if (device == null) {
            device = searchFoundDevicesF(FILTER_NAME);
            if (device == null) {
                mBluetoothAdapter.startDiscovery();
                setConnectDeviceMAC(null);
                return false;
    ...
    private BluetoothDevice searchPairedDevicesF(String name) {
    ...
    private BluetoothDevice searchFoundDevicesF(String name) {
        if (TextUtils.isEmpty(name))
            return null;
